...
import { buildReviewModel, buildReviewModelFromTree, } from "../src/diff/model.js";
const usage = {
    input: 0,
    output: 0,
    cacheRead: 0,
...
describe("buildReviewModel", () => {
    it("groups edit diffs by turn, file, and hunk", () => {
        ...
        expect(model.turns[0]?.files[0]?.hunks.map((hunk) => hunk.jumpLine)).toEqual([2, 11]);
    });
    it("keeps the full normalized user prompt for expanded action views", () => {
        const longPrompt = `${"summarize these changes ".repeat(12)}final instruction`.trim();
        const entries = [
            {
                type: "message",
                id: "u1",
    ...
    it("builds a compressed diff-producing turn tree from session branches", () => {
        ...
        expect(model.activeTurnIds.map((turnId) => model.turns.find((turn) => turn.id === turnId)?.prompt)).toEqual(["create base", "branch beta"]);
