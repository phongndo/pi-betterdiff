...
import { gitBranchDiffArgs, gitDiffArgs, parseGitBranchReviewModel, parseGitChangesReviewModel, } from "../src/diff/git.js";
const modifiedPatch = [
...
describe("parseGitChangesReviewModel", () => {
    ...
    it("keeps an empty staged section above non-empty unstaged changes", () => {
        ...
        expect(model.turns[1]?.prompt).toBe("Unstaged changes — index → working tree");
    ...
    it("creates metadata hunks for binary or mode-only diffs", () => {
        ...
        expect(hunk?.bodyLines).toContain(" Binary files /dev/null and b/image.png differ");
    });
...
describe("parseGitBranchReviewModel", () => {
    it("parses current-branch-vs-base branch diffs", () => {
        ...
        expect(model.mode.description).toBe("merge-base(main, feature/foo) → feature/foo");
        expect(model.turns[0]?.prompt).toBe("Current branch vs main — main...feature/foo");
        expect(model.totalFiles).toBe(1);
        expect(model.totalHunks).toBe(1);
        expect(model.additions).toBe(2);
        expect(model.removals).toBe(1);
