import { gitBranchDiffArgs, gitDiffArgs, parseGitBranchReviewModel, parseGitChangesReviewModel, } from "./diff/git.js";
import { buildReviewModelFromTree } from "./diff/model.js";
import { DiffReviewComponent, } from "./render/diff-review-ui.js";
export const BETTERDIFF_EXTENSION_STAGE = "ui-prototype";
...
export default function betterDiffExtension(pi) {
    async function openDiffReview(ctx, initialRequest) {
        if (!ctx.hasUI) {
        ...
        try {
            ...
            ctx.ui.notify(`Failed to load diff: ${error instanceof Error ? error.message : String(error)}`, "warning");
            return;
        }
        const action = await ctx.ui.custom((tui, theme, keybindings, done) => {
            return new DiffReviewComponent(model, ctx.cwd, tui, theme, keybindings, done, (request) => loadReviewModel(request, ctx), () => loadGitBranchRefs(ctx));
        });
        if (!action || action.type === "close")
            return;
        await handleDiffReviewAction(action, ctx);
    }
    async function loadReviewModel(request, ctx) {
        if (request.kind === "session-turns") {
            return buildReviewModelFromTree(ctx.sessionManager.getTree(), ctx.sessionManager.getLeafId());
        }
        ...
        return loadGitBranchReviewModel(ctx, "git-branch-selected", request.baseRef);
    }
    async function loadGitChangesReviewModel(ctx) {
        ...
        if (stagedResult.code !== 0) {
            throw new Error(stagedResult.stderr.trim() ||
                `git staged diff exited with status ${stagedResult.code}`);
        }
        if (unstagedResult.code !== 0) {
            throw new Error(unstagedResult.stderr.trim() ||
                `git unstaged diff exited with status ${unstagedResult.code}`);
        }
        return parseGitChangesReviewModel(stagedResult.stdout, unstagedResult.stdout);
    }
    async function loadGitBranchReviewModel(ctx, kind, baseRef) {
    ...
    async function loadGitBranchRefs(ctx) {
    ...
    async function findDefaultBaseRef(ctx) {
        ...
        for (const candidate of candidates) {
            if (await gitRefExists(ctx, candidate))
                return candidate;
        }
        throw new Error("Could not find main/master. Use 'Current branch vs selected branch…' instead.");
    }
    async function getCurrentGitRef(ctx) {
        const branch = (await gitStdout(ctx, ["rev-parse", "--abbrev-ref", "HEAD"])).trim();
        if (branch && branch !== "HEAD")
            return branch;
        return (await gitStdout(ctx, ["rev-parse", "--short", "HEAD"])).trim();
    }
    async function verifyGitRef(ctx, ref) {
        if (await gitRefExists(ctx, ref))
            return;
        throw new Error(`Invalid git ref: ${ref}`);
    }
    async function ensureMergeBase(ctx, baseRef, currentRef) {
    ...
    async function gitRefExists(ctx, ref) {
    ...
    async function tryGitStdout(ctx, args) {
    ...
    async function gitStdout(ctx, args) {
        const result = await pi.exec("git", args, {
            cwd: ctx.cwd,
            timeout: 10_000,
        });
    ...
    function parseInitialDiffMode(args) {
        const raw = args.trim();
    ...
    async function handleDiffReviewAction(action, ctx) {
        if (action.type === "summarize") {
            await sendSummaryRequest(action.summary, action.custom, ctx);
        }
    }
    async function sendSummaryRequest(summary, custom, ctx) {
        let customInstructions;
        if (custom) {
            customInstructions = await ctx.ui.editor("Custom BetterDiff summary instructions", "Focus on behavior changes, risks, and follow-up tests.");
    ...
    function buildSummaryPrompt(summary, customInstructions) {
        return [
    ...
    pi.registerCommand("diff", {
        ...
        handler: async (args, ctx) => {
            ...
            if (!mode) {
                ctx.ui.notify("Usage: /diff [session|git|branch|branch <base-ref>]. You can also press m inside Better Diff to switch modes.", "warning");
                return;
            }
